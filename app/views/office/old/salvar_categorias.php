...
if ($vformACAO == "novo") {	
	...
	if ($vformTIPO == "0") {
		...
		$dbVALORES .= ",0" . $vformSELECTSUBCATEGORIA;
		$dbVALORES .= ",'subgrupo'";
...
if ($vformACAO == "atualizar") {
	...
	$vConexao->query($dbALT . "id_pai=0" . $vformSELECTSUBCATEGORIA . $dbWHERE) or die (mysql_error());
...
<html lang="pt-br">
	...
	<body>
		...
		if ($vSALVAR) {
			...
			if ($vformACAO == "novo") {	
				echo 'top.document.frmCadCategorias.formNOME.value = "";';
				echo 'top.document.frmCadCategorias.formSELECTSUBCATEGORIA.value = "";';
