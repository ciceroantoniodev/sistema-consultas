			...
			<div align="center">
				<div class="area-litagens">
					<div align="left" style="margin: 30px;">
						<div style="font-size: 13px"><a href="javascript: showDIRECT('', 'inicio.php?idu=<?php echo fId("c", $vgetIDUSUARIO) ?>', 'areaConteudo')" class="ltopo">IN&Iacute;CIO</a> / GERENCIAMENTO DE FORNECEDORES</div><br />
						...
						echo '<div class="floatLEFT"><div class="Titulo-Interno">Gerenciamento de Fornecedores</div></div>';
						...
						echo '	</form>';
						echo '</div>';
						echo '<div class="floatRIGHT"><button class="btn-novo" onclick="showDIRECT(\'\', \'cad_fornecedores.php?idu=' . fId("c", $vgetIDUSUARIO) . '\', \'areaConteudo\')">Novo</button></div>';
						...
						$queryFornecedores = $vConexao->query("SELECT * FROM sysc_fornecedores ORDER BY nome") or die("Falha na execução da consulta.");
							...
							while ($reFornecedores = mysqli_fetch_assoc($queryFornecedores)) {
								...
								echo '<input type="checkbox" name="formEXCLUIR" value="' . $reFornecedores['id'] . '" />';
								echo '</div></td>';
								
								echo '<td class="area-grid-cel"><div align="center">';
									echo '<a href="javascript: showDIRECT(\'\', \'cad_fornecedores.php?idu=' . fId("c", $vgetIDUSUARIO) . '&acao=alterar&ida=' . $reFornecedores['id'] . '\', \'areaConteudo\')" class="grid">';
									echo StrZero($reFornecedores['id'], 6);
									echo '</a>';
								echo '</div></td>';

								echo '<td class="area-grid-cel">';
									echo '<a href="javascript: showDIRECT(\'\', \'cad_fornecedores.php?idu=' . fId("c", $vgetIDUSUARIO) . '&acao=alterar&ida=' . $reFornecedores['id'] . '\', \'areaConteudo\')" class="grid">';
									echo trim($reFornecedores['nome']);
									echo '</a>';
								echo '</td>';

								echo '<td class="area-grid-cel">';
									echo '<a href="javascript: showDIRECT(\'\', \'cad_fornecedores.php?idu=' . fId("c", $vgetIDUSUARIO) . '&acao=alterar&ida=' . $reFornecedores['id'] . '\', \'areaConteudo\')" class="grid">';
									echo $reFornecedores['tipo'];
									echo '</a>';
								echo '</td>';

								echo '<td class="area-grid-cel">';
									echo '<a href="javascript: showDIRECT(\'\', \'cad_fornecedores.php?idu=' . fId("c", $vgetIDUSUARIO) . '&acao=alterar&ida=' . $reFornecedores['id'] . '\', \'areaConteudo\')" class="grid">';
									echo $reFornecedores['cidade'] . '-' . $reFornecedores['estado'];
									echo '</a>';
								echo '</td>';
								...
								echo '<td class="area-grid-cel">';
									...
									echo '(' . $reFornecedores['dddfone1'] . ') ' . $reFornecedores['fone1'];
									
									echo '</a>';
								echo '</td>';

								echo '<td class="area-grid-cel borderRIGHT">';
									echo '<div align="center"><a href="javascript: showDIRECT(\'\', \'ger_images.php?idu=' . fId("c", $vgetIDUSUARIO) . '&o=fornecedores&ida=' . $reFornecedores['id'] . '\', \'areaConteudo\')" class="grid">';
