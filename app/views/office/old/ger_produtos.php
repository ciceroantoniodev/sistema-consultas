			...
			<div align="center">
				<div class="area-litagens">
					<div align="left" style="margin: 30px;">
						<div style="font-size: 13px"><a href="javascript: showDIRECT('', 'inicio.php?idu=<?php echo fId("c", $vgetIDUSUARIO) ?>', 'areaConteudo')" class="ltopo">IN&Iacute;CIO</a> / GERENCIAMENTO DE PRODUTOS</div><br />
						...
						echo '<div class="floatLEFT"><div class="Titulo-Interno">Gerenciamento de Produtos</div></div>';
						...
						echo '		<input type="hidden" name="formDELTABELA" value="sysc_produtos" />';
						...
						echo '<td align="center" class="area-grid-cabeca">FORNECEDOR</td>';
						echo '<td align="center" class="area-grid-cabeca">A&Ccedil;&Atilde;O</td></tr>';

						$queryProdutos = $vConexao->query("SELECT * FROM sysc_produtos ORDER BY nome") or die("Falha na execução da consulta.");
							...
							while ($reProdutos = mysqli_fetch_assoc($queryProdutos)) {
								...
								echo '<input type="checkbox" name="formEXCLUIR" value="' . $reProdutos['id'] . '" />';
								echo '</div></td>';
								
								echo '<td class="area-grid-cel"><div align="center">';
									echo '<a href="javascript: showDIRECT(\'\', \'cad_produtos.php?idu=' . fId("c", $vgetIDUSUARIO) . '&acao=alterar&ida=' . $reProdutos['id'] . '\', \'areaConteudo\')" class="grid">';
									echo StrZero($reProdutos['id'], 6);
									echo '</a>';
								echo '</div></td>';

								echo '<td class="area-grid-cel">';
									echo '<a href="javascript: showDIRECT(\'\', \'cad_produtos.php?idu=' . fId("c", $vgetIDUSUARIO) . '&acao=alterar&ida=' . $reProdutos['id'] . '\', \'areaConteudo\')" class="grid">';
									echo trim($reProdutos['nome']);
									echo '</a>';
								echo '</td>';

								echo '<td class="area-grid-cel">';
									echo '<a href="javascript: showDIRECT(\'\', \'cad_produtos.php?idu=' . fId("c", $vgetIDUSUARIO) . '&acao=alterar&ida=' . $reProdutos['id'] . '\', \'areaConteudo\')" class="grid">';
									echo $reProdutos['link'];
									echo '</a>';
								echo '</td>';

								echo '<td class="area-grid-cel">';
									echo '<a href="javascript: showDIRECT(\'\', \'cad_produtos.php?idu=' . fId("c", $vgetIDUSUARIO) . '&acao=alterar&ida=' . $reProdutos['id'] . '\', \'areaConteudo\')" class="grid">';
									echo substr($reProdutos['fornecedor'], 0, 30) . '...';
									echo '</a>';
								echo '</td>';
								
								echo '<td class="area-grid-cel borderRIGHT">';
									echo '<div align="center"><a href="javascript: showDIRECT(\'\', \'ger_images.php?idu=' . fId("c", $vgetIDUSUARIO) . '&o=produtos&ida=' . $reProdutos['id'] . '\', \'areaConteudo\')" class="grid">';
