...
if ($vgetACAO != "") {
	if ($vgetACAO == "excluir") {
		...
		if (file_exists("../documentos/noticias/imagens/" . $vgetFOTO)) {
			...
			$vConexao->query("UPDATE sysc_noticias SET foto2='' WHERE id=" . $vgetID_NOTICIA) or die (mysql_error());
...
<html>
	...
	<body bgcolor="#FBF8EF">
		...
		if ($vArquivoAcao == "enviado") {
			...
			echo 'parent.document.getElementById("FotoTexto").value = "' . $vgetFOTO . '";';
			echo '</script>';

		} else if ($vArquivoAcao == "excluido") {
			echo '<script language="Javascript" type="text/javascript">';
			echo 'parent.document.getElementById("FotoTexto").value = "";';
			echo '</script>';
		...
		?>
		<div align="center">
		<form action="noticia_fototexto.php?ida=<?php echo $vgetID_NOTICIA ?>&idu=<?php echo $vgetIDUSUARIO ?>&tp=<?php echo $vgetTIPO ?>" method="post" style="margin: 0px" enctype="multipart/form-data">
			<strong>FOTO DO TEXTO:</strong><br><br>
			<?php
			if ($vgetID_NOTICIA != "") {
				...
				if ($syscPOSICAO > 0) {	
					...
					echo '<br><a href="noticia_fototexto.php?id=' . $vgetID_NOTICIA . '&file=' . $vgetFOTO . '&acao=excluir" class="grid">[ Excluir Imagem ]</a>';
