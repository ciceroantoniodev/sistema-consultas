...
$vDelTabela = "sysc_mensagensenviadas";
$vDelCampo = "assunto";

$vStatus = isset($_GET['status']) ? $_GET['status'] : NULL;

$vTituloSecao = "GERENCIAMENTO DE MENSAGENS";
...
echo '<tr><td class="form-cadastros-head" colspan="7"><a href="javascript: history.go(-' . $vBotaoVoltar . ')"><div class="botao-voltar"><img src="images/botao_voltar.gif" height="30" /></div></a><div align="center">' . $vTituloSecao . '</div></td></tr>';
echo '<tr><td class="area-grid-barra" colspan="7">';
echo '<div class="floatRIGHT"><form name="frmGridBuscar" onSubmit="return false">Buscar: <input type="text" name="formBUSCAR" size="30" class="form_buscar" /><input type="button" value="Ok!" class="botao-ok" onClick="fAjaxDirecionar(\'grid_mensagens.php?id=' . $vgetIDUSUARIO. '&status=' . $vStatus . '&key=A\')" /></form></div>';

//if (strpos("ABC" . $vusuarioACESSOS, "4") > 0) {
	echo '<div class="floatRIGHT"><form action="excluir_registros.php?id=' . $vgetIDUSUARIO . '" method="post" name="frmGridExcluir"><input type="hidden" name="formDELTABELA" value="" /><input type="hidden" name="formDELCAMPO" value="" /><input type="hidden" name="formDEL" value="" /><input type="button" value="  Excluir Seleção " class="botao-excluir" onClick="fExcluirRegistros(\'' . $vDelTabela . '\',\'' . $vDelCampo . '\')" /></form></div>';
	echo '<div class="floatRIGHT"><a href="cad_mensagens.php?id=' . $vgetIDUSUARIO . '&tp=' . $vgetTIPO . '" class="rodape" target="main"><div class="botao-novo">&nbsp;&nbsp;&nbsp;Nova Mensagem&nbsp;&nbsp;&nbsp;</div></a></div>';
...
<script type="text/javascript">showDIRECT("", "grid_mensagens.php?id=<?php echo$vgetIDUSUARIO ?>&status=<?php echo $vStatus ?>&key=", "areaDIRECT");</script>
