...
<body>
<?php
$dbSQL = "SELECT * FROM sysc_cadastrofranquias";

if ($vgetKEY != $vgetQRY) {
	$dbSQL .= " WHERE (id_cliente=" . $vusuarioIDC . ") AND ((nome LIKE '%" . $vgetQRY . "%') or (sobrenome LIKE '%" . $vgetQRY . "%') or (cpf LIKE '%" . $vgetQRY . "%') or (cidade LIKE '%" . $vgetQRY . "%') or (estado LIKE '%" . $vgetQRY . "%') or (fone LIKE '%" . $vgetQRY . "%') or (celular LIKE '%" . $vgetQRY . "%') or (email_proprio LIKE '%" . $vgetQRY . "%')) order by nome";

} else {
	$dbSQL .= " WHERE (id_proprietario=" . $vgetIDUSUARIO . ") ORDER BY nome";
...
echo '<td align="center" class="area-grid-cabeca">NOME DO PROPRIETARIO</td>';
echo '<td align="center" class="area-grid-cabeca">CIDADES QUE ABRANGE</td>';
