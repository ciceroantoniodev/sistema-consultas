...
$vTituloSecao = "Cadastrar Novo Usu&aacute;rio";
...
$vformCPF = "";
$vformRG = "";
$vformDTNASCIMENTODIA = "00";
$vformDTNASCIMENTOMES = "00";
$vformDTNASCIMENTOANO = "0000";
$vformENDERECO_NUM = "";
$vformBAIRRO = "";
$vformESTADO = "";
$vformDDDFONE = "";
...
$queryCargos = $vConexao->query("SELECT * FROM sysc_cargos WHERE tipo='cargo' AND ativo='S'") or die("Falha na execução da consulta.");
	$arrayCargos = Array();
	$i = 0;
	
	while ($reCargos = mysqli_fetch_assoc($queryCargos)){
		$arrayCargos[$i] = Array("Id"=>$reCargos['id'], "Cargo"=>$reCargos['cargo'], "Pessoas"=>$reCargos['pessoas'], "Ativo"=>$reCargos['ativo']);
...
if ($vgetAcao == "alterar") {
	$queryUsuarios = $vConexao->query("SELECT * FROM sysc_usuarios WHERE id=" . $vgetID) or die("Falha na execução da consulta.");
		...
		while ($reUsuarios = mysqli_fetch_assoc($queryUsuarios)){
			$vformUSUARIO = $reUsuarios['usuario'];
			$vformSENHA = $reUsuarios['senha'];
			...
			$vformID_CARGO = $reUsuarios['id_cargo'];
			$vformCARGO = $reUsuarios['cargo'];
			
			$vformOBS = $reUsuarios['obs'];
			
		}
	mysqli_free_result($queryUsuarios);
	
	$vTituloSecao = "Alterar Dados do Usu&aacute;rio";
				...
				<div class="area-quero-apostas">
					<div align="left" style="margin: 30px;">
						...
						<div id="area-cadastro">   
							<form method="post" action="salvar_usuarios.php?idu=<?php echo fId("c", $vgetIDUSUARIO) ?>" target="direcionar" id="frmCadastro" name="frmCadastro" onSubmit="return fValidaCadastro()">
								<input type="hidden" name="formACAO" value="<?php echo $vgetAcao ?>" />
								<input type="hidden" name="formIDUSUARIO" value="<?php echo $vgetID ?>" />
								...
								<div id="area-cadastro-left">
									...
									<div>
										<select name="formCARGO" id="idCARGO" class="form-edit" tabindex="1">
											<?php
											$vEcho = '';

											for ($i = 0; $i < count($arrayCargos); $i++) {
												if ($vformID_CARGO == $arrayCargos[$i]['Id']) {
													$vEcho .= '<option selected="selected" value="' . $arrayCargos[$i]['Id'] . '|'. $arrayCargos[$i]['Cargo'] . '" >' . $arrayCargos[$i]['Cargo'] . '</option>';

												} else {
													$vEcho .= '<option value="' . $arrayCargos[$i]['Id'] . '|'. $arrayCargos[$i]['Cargo'] . '" >' . $arrayCargos[$i]['Cargo'] . '</option>';
								...
								<div id="area-cadastro-right">
									<?php
									if ($vgetAcao == "alterar") {
										echo '<br/><br/><span class="fonte18 CorRed">C&Oacute;DIGO: <strong>' . StrZero($vgetID, 6) . '</strong></span>';
									}
									?>
								</div>
								...
								<div id="area-cadastro-left">
									...
									<input type="text" name="formEMAILADICIONAL" id="idEMAILADICIONAL" class="form-edit" tabindex="24" maxlength="150" value="<?php echo $vformEMAILADICIONAL ?>" />
								...
								<div id="area-cadastro-right">					
									...
									<input type="text" name="formSKYPE" id="isSKYPE" class="form-edit" tabindex="25" maxlength="50" value="<?php echo $vformSKYPE ?>" />
								</div>
								...
								<div>
									<div class="form-area-campos">
										<table border="0" cellspacing="0" cellpadding="0">
											...
											<tr>
												<td width="45"><input type="text" name="formDDDCELULAR1" id="idDDDCELULAR1" onKeyPress="return fApenasNumero(event)" class="form-edit-fone1" tabindex="26" maxlength="2" value="<?php echo $vformDDDCELULAR1 ?>" /></td>
												<td width="105"><input type="text" name="formCELULAR1" id="idCELULAR1" onKeyPress="return fApenasNumero(event)" class="form-edit-fone2" tabindex="26" maxlength="20" value="<?php echo $vformCELULAR1 ?>" /></td>
												<td width="80">
													<select name="formOPERADORA1" id="idOPERADORA3" class="form-edit-fone3" tabindex="27">
														<?php
														$vEcho = "";
												...
												<td width="45"><input type="text" name="formDDDCELULAR2" id="idDDDCELULAR2" onKeyPress="return fApenasNumero(event)" class="form-edit-fone1" tabindex="28" maxlength="2" value="<?php echo $vformDDDCELULAR2 ?>" /></td>
												<td width="105"><input type="text" name="formCELULAR2" id="idCELULAR2" onKeyPress="return fApenasNumero(event)" class="form-edit-fone2" tabindex="29" maxlength="20" value="<?php echo $vformCELULAR2 ?>" /></td>
												<td width="80">
													<select name="formOPERADORA2" id="idOPERADORA2" class="form-edit-fone3" tabindex="30">
														<?php
														$vEcho = "";
												...
												<td width="45"><input type="text" name="formDDDCELULAR3" id="idDDDCELULAR3" onKeyPress="return fApenasNumero(event)" class="form-edit-fone1" tabindex="31" maxlength="2" value="<?php echo $vformDDDCELULAR3 ?>" /></td>
												<td width="105"><input type="text" name="formCELULAR3" id="idCELULAR3" onKeyPress="return fApenasNumero(event)" class="form-edit-fone2" tabindex="32" maxlength="20" value="<?php echo $vformCELULAR3 ?>" /></td>
												<td width="100">
													<select name="formOPERADORA3" id="idOPERADORA3" class="form-edit-fone3" tabindex="33">
														<?php
														$vEcho = "";
												...
												<td width="155"><input type="text" name="formWHATSAPP" id="idWHATSAPP" class="form-editMedium" tabindex="34" maxlength="15" value="<?php echo $vformWHATSAPP ?>" /></td>
											</tr>
										</table>
									</div>
								</div>
								...
								<div>
									<label for="Skype">Observa&ccedil;&atilde;o</label>
									<textarea name="formOBS" id="isOBS" class="form-edit" rows="5" tabindex="35"><?php echo $vformOBS ?></textarea>
								</div>
								
								<div class="clear"><br /><br /></div>
								
								<div class="cadastro-titulo">DADOS DE ACESSO</div>
								
								<div class="clear"><br /></div>
								
								<div id="boxEIXO"></div>

								<input type="hidden" name="formUSUARIOATUAL" value="<?php echo $vformUSUARIO ?>" />
								<input type="hidden" name="formSENHAATUAL" value=""<?php echo $vformSENHA ?>"" />
								
								<div id="area-cadastro-left">
									<label for="idUSUARIO">Usu&aacute;rio</label>
									<input type="text" name="formUSUARIO" id="idUSUARIO" class="form-edit" tabindex="36" maxlength="100"  value="<?php echo $vformUSUARIO ?>" />
						  
									<label for="idSENHA">Senha</label>
									<div class="clear"></div>
									<input type="password" name="formSENHA" id="idSENHA" class="form-editMetade" tabindex="37" maxlength="100" value="" />
						  
									<label for="idSENHAB">Repitir Senha</label>
									<div class="clear"></div>
									<input type="password" name="formSENHAB" id="idSENHAB" class="form-editMetade" tabindex="38" maxlength="100" value="" />
								</div>
								...
								<div>
									<div style="float: left"><br /><br /><input type="submit" id="button" value="   ATUALIZAR  " tabindex="38" onClick="fImagemLoad()" class="formSUBMIT" /></div>
								</div>
								
								<iframe src="vazio.php" scrolling="yes" frameborder="0" name="direcionar" style="border: #DDDDDD 1px solid; overflow:hidden; width:1px; height:1px;" allowTransparency="true"></iframe>
